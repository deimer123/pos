...
- CI en GitHub Actions (`.github/workflows/ci.yml`): corre en cada push/PR a `main` — lint de sintaxis PHP + `npm run build` (assets) + suite completa de tests (Pest) contra un MySQL efímero. Es solo validación, NO despliega nada.
- El despliegue sigue siendo manual: hay que entrar al servidor por SSH y correr:
  ```bash
  git pull origin main
  php artisan optimize:clear
  ```
- **Si el cambio toca CSS/JS/Blade con `@vite(...)`** (no solo PHP), además hay que recompilar los assets en el servidor, porque `public/build` no está en el repo (está en `.gitignore`):
  ```bash
  npm ci
  npm run build
  ```
  Sin este paso, `git pull` no actualiza lo que el navegador realmente carga — se queda con los assets viejos ya compilados.

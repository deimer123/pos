<div style="height:100%; display:flex; flex-direction:column; background:#f0fdf4;">
        ...
        <div style="background:white;border-radius:16px;width:100%;max-width:440px;box-shadow:0 20px 60px rgba(0,0,0,.3);">
            ...
            <div style="padding:20px;">
                <div style="background:#f0fdf4;border-radius:10px;padding:12px 16px;margin-bottom:16px;">
                    ...
                    <div style="font-size:11px;color:#6b7280;margin-top:4px;">Orden #{{ str_pad($orden->numero_orden,4,'0',STR_PAD_LEFT) }} · {{ trim(($orden->marca ?? '').' '.($orden->modelo ?? '')) ?: 'Equipo' }} · {{ $orden->cliente_nombre }}</div>
                </div>
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <input wire:model="obsFactura" type="text" placeholder="Ej: Cambio de pantalla + batería..."
    ...
    <div style="background:#0f766e;color:white;padding:10px 16px;flex-shrink:0;">
        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
            {{-- Equipo destacado --}}
            <div style="background:white;color:#0f766e;border-radius:8px;padding:4px 14px;font-size:16px;font-weight:900;">
                {{ trim(($orden->marca ?? '').' '.($orden->modelo ?? '')) ?: 'Equipo' }}
            </div>
            <div style="flex:1;min-width:0;">
                <div style="font-size:13px;font-weight:700;">
                    @if($orden->imei_serial) IMEI/Serial: {{ $orden->imei_serial }} @endif
            ...
            <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                @php
                    ...
                    $est = $estadosServicioTecnico[$orden->estado] ?? $estadosServicioTecnico['pendiente'];
                @endphp
    ...
    <div style="flex:1;overflow:hidden;display:flex;">
        ...
        <div style="width:55%;border-right:2px solid #d1fae5;display:flex;flex-direction:column;background:white;">
            ...
            {{-- ─ Video / evidencia (unico frente a Taller) ─ --}}
            <div style="border-top:2px solid #d1fae5;padding:12px 14px;flex-shrink:0;background:#f8fafc;max-height:220px;overflow-y:auto;">
                <div style="font-size:11px;font-weight:700;color:#0f766e;text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px;">🎥 Video (ej. falla del equipo al ingresar)</div>
                <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-start;">
                    @forelse($orden->videos ?? [] as $i => $video)
                    <div style="position:relative;">
                        <video src="{{ Storage::disk('public')->url($video) }}" controls preload="metadata"
        ...
        <div style="flex:1;display:flex;flex-direction:column;background:#f8fafc;">
            ...
            <div style="border-top:2px solid #d1fae5;padding:12px 14px;background:white;flex-shrink:0;">
                @php
                    $totalRepuestos  = $orden->items->where('tipo','repuesto')->sum('subtotal');
                    $totalManoObra   = $orden->items->where('tipo','mano_obra')->sum('subtotal');
                    $totalTerceros   = $orden->items->where('tipo','tercero')->sum('subtotal');
                @endphp

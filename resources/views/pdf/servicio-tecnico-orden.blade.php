...
<style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .imei { font-size:16px; font-weight:900; color:#0f766e; letter-spacing:.03em; }
...
<div class="header">
    <h1>{{ $config?->nombre_empresa ?? 'SERVICIO TECNICO' }}</h1>
...
<div class="seccion">
    ...
    <div class="grid2">
        <div class="col">
            <div class="campo"><div class="label">Marca / Modelo</div><div class="valor">{{ trim(($orden->marca ?? '') . ' ' . ($orden->modelo ?? '')) ?: '—' }}</div></div>
            <div class="campo" style="margin-top:6px;"><div class="label">IMEI / Serial</div><div class="imei">{{ $orden->imei_serial ?: '—' }}</div></div>
        </div>
        <div class="col">
    ...
    <table>
        ...
        <tbody>
            ...
            <tr class="tr-total">
                ...
                <td style="text-align:right;">${{ number_format($orden->items->sum('subtotal'), 0, ',', '.') }}</td>
            </tr>
        </tbody>
    </table>
    @else
...
<div class="seccion">
    <div class="seccion-titulo">🎥 Anexos - Video</div>
    <div style="color:#6b7280; font-size:11px;">{{ count($orden->videos) }} video(s) anexado(s) -- disponibles desde el sistema (un PDF no reproduce video).</div>
</div>
@endif

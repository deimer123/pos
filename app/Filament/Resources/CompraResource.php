...
public static function table(Tables\Table $table): Tables\Table
{
    ...
    if (!$tieneFiltros) {
        ...
        ->columns([
         ...
         ->actions([
                Action::make('generar_etiquetas')
                    ->label('Generar Etiquetas')
                    ->form(function (Action $action) {
                        return [
                            Forms\Components\Select::make('tipo_codigo')
                                ->options([
                                    'barcode' => 'Código de Barras',
                                    'qr' => 'Código QR',
                                ])
                                ->required(),
                            Forms\Components\Select::make('tamaño')
    ->options([
        '2x50' => '2 etiquetas de 50 × 25 mm',
        '3x32' => '3 etiquetas de 32 × 25 mm',
    ])
    ->required(),
                            Forms\Components\TextInput::make('almacen')
    ...
    ->default(function ($record) {
        if (!$record || !$record->exists || !method_exists($record, 'productos')) {
            return [];
        }
        ...
        return $record->productos()
            ...
            ->toArray();
                  ...
                  ->action(function (array $data, Compra $record) {
                    
    [$ancho, $papel_ancho, $formato] = match ($data['tamaño']) {
    '2x50' => [50, 108, '2x50'],
    '3x32' => [32, 103, '3x32'],
};
    ...
    foreach ($data['productos'] as $prod) {
        // Validación segura: asegurarse de que tenga las claves necesarias
        if (!isset($prod['product_id']) || !isset($prod['nombre']) || !isset($prod['cantidad'])) {
            continue;
        }

        // Validación numérica de cantidad
        $cantidad = is_numeric($prod['cantidad']) ? (int)$prod['cantidad'] : 1;
    ...
    // Ordenar por nombre
    usort($etiquetas, fn($a, $b) => strcmp($a['nombre'], $b['nombre']));

   $pdf = Pdf::loadView('pdf.etiquetas', [
    'etiquetas'    => collect($etiquetas),

...
use App\Support\VariantCodeGenerator;
...
// Cargue masivo de productos con lotes (fecha de vencimiento propia por
// lote), para empresas con el flag usa_lotes activo -- droguería, víveres o
// cualquier otro tipo de negocio que lo prenda. Cada fila es UN lote; varias
// filas con el mismo "Nombre del producto" se agrupan en un solo producto
// con varios lotes. Calco exacto de ProductoRopaBulkImport (talla/color),
// cambiando la combinacion talla+color por lote+fecha_vencimiento. Igual que
// CompraBulkImport: primero se valida TODO (pase 1, sin tocar la base de
// datos) y solo si no hay ningun error se crea algo (pase 2) -- asi se
// puede corregir el excel y volver a subir el mismo archivo sin dejar
// productos a medias.
class ProductoLoteBulkImport implements ToCollection, WithHeadingRow, WithMultipleSheets
{
    protected int $creados = 0;
    protected int $lotesCreados = 0;
    ...
    private function agrupar(Collection $rows): array
    {
        ...
        foreach ($rows as $index => $rawRow) {
            ...
            // realmente escribe (Nombre/Lote/Fecha), no por todas. Precio de
            // Venta trae formula en las 500 filas de la plantilla (ver
            // ProductoLoteBulkTemplatePlantillaSheet), asi que esa celda
            // nunca esta "vacia" de verdad -- si se revisaran todas las
            // columnas, una fila vacia de verdad salia marcada como "con
            // datos" por error (mismo cuidado que ya tiene ProductoRopaBulkImport).
            $nombreRaw = trim((string) ($row['nombre_del_producto'] ?? ''));
            $loteRaw = trim((string) ($row['lote'] ?? ''));
            $fechaRaw = $row['fecha_de_vencimiento'] ?? null;

            if ($nombreRaw === '' && $loteRaw === '' && $this->fecha($fechaRaw) === null) {
                continue;
            }
            ...
            $fechaVencimiento = $this->fecha($fechaRaw);

            if ($lote === '') {
                $this->errores[] = "Fila {$numeroFila} ({$nombre}): falta el número de Lote.";
                continue;
            }

            if ($fechaVencimiento === null) {
                $this->errores[] = "Fila {$numeroFila} ({$nombre}): falta la Fecha de vencimiento (o no se pudo leer).";
                continue;
            }
            ...
            $lotesClaves = collect($grupos[$clave]['lotes'])->map(fn ($l) => mb_strtolower($l['lote']));
            if ($lotesClaves->contains(mb_strtolower($lote))) {
                $this->errores[] = "Fila {$numeroFila} ({$nombre}): el lote \"{$lote}\" esta repetido.";
                continue;
            }
     ...
     * validos. Aqui si se crean los productos y sus lotes.
     */
    private function confirmar(array $grupos): void
    {
        ...
        DB::transaction(function () use ($grupos, &$siguienteCodigo) {
            foreach ($grupos as $grupo) {
                ...
                foreach ($grupo['lotes'] as $l) {
                    // Codigo propio para cada lote (producto + lote), asi
                    // todos quedan con codigo de barras sin obligar a
                    // cargarlo aparte despues (mismo mecanismo que
                    // ProductResource::sugerirCodigoLoteSiFalta()).
                    $codigo = VariantCodeGenerator::sugerir(
                    ...
                    // Compras (que reconoce Lote/Fecha de vencimiento del
                    // lote).
                    ProductoLote::create([
                        'empresa_id' => $this->empresaId,
                        'product_id' => $producto->id,
    ...
    public function resumen(): array
    {
        return [
            ...
            'lotes_creados' => $this->lotesCreados,
